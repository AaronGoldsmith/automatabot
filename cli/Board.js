...
class Board
{
  ...
  getNextRow(n){
    ...
    return cells.map((_, i) => {
      ...
      const neighborStates = neighbors.map(neighbor => this.board[neighbor.x][neighbor.y])
  ...
  getNeighbors(row, col){
    ...
    for (let i = row - 1; i <= row + 1; i++) {
      for (let j = col - 1; j <= col + 1; j++) {
        if (row === i && col === j) { } //console.log("\n")
        else if (i >= 0 && j >= 0 && i < size && j < this.board[j].length) neighbors.push({ x: i, y: j })
      }
    }
    return neighbors;
  }
  toString(){
    return this.board.map(row => row.join("").replace(/0/g, " ").replace(/1/g, "⦿"))
  }
